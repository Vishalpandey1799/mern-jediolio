...
const DevPortfolio = () => {
  ...
  const getPlatformIcon = (platform) => {
    const iconName = `fa${platform.charAt(0).toUpperCase()}${platform.slice(1).toLowerCase()}`;
  ...
  return (
    <div className="flex h-[800px] items-center justify-center" onClick={navigator}>
      <div className="w-80 bg-white bg-opacity-90 rounded-2xl shadow-lg overflow-hidden mt-15 transition-transform transform hover:-translate-y-2 hover:shadow-xl">
        <div className="p-6 text-center bg-gradient-to-r from-purple-600 to-pink-500">
          ...
          />
          <h1 className="text-white text-2xl font-bold mt-4">{userData.name}</h1>
                ...
                return (
                  ...
                  >
                    {typeof platformIcon === "string" ? (
                      ...
                      <FontAwesomeIcon icon={platformIcon} size="sm" className="text-white" />
                    )}
                  </a>
                );
              })}
